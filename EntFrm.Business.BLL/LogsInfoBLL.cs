...
{
    public class LogsInfoBLL
    {

        private string connStr;
        private string appCode;

        public  LogsInfoBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               ILogsInfo infoDAL = LogsInfoFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" LogNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(LogsInfo info)
        {
            try
            {
            if (!ValidateRepeat(info.sLogNo))
            {
                ILogsInfo infoDAL = LogsInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(LogsInfo info)
        {
            try
            {
                     ILogsInfo  infoDAL = LogsInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  LogsInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                ILogsInfo infoDAL = LogsInfoFactory.Create(this.connStr, this.appCode);
                LogsInfoCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public LogsInfoCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               ILogsInfo infoDAL = LogsInfoFactory.Create(this.connStr, this.appCode);
        ...
        public LogsInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
