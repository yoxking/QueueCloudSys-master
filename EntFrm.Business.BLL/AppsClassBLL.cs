...
{
    public class AppsClassBLL
    {

        private string connStr;
        private string appCode;

        public  AppsClassBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IAppsClass infoDAL = AppsClassFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(AppsClass info)
        {
            try
            {
        ...
        public  bool UpdateRecord(AppsClass info)
        {
            try
            {
                     IAppsClass  infoDAL = AppsClassFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  AppsClass GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IAppsClass infoDAL = AppsClassFactory.Create(this.connStr, this.appCode);
                AppsClassCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public AppsClassCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IAppsClass infoDAL = AppsClassFactory.Create(this.connStr, this.appCode);
        ...
        public AppsClassCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
