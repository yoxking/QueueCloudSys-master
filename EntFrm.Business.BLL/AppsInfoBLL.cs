...
{
    public class AppsInfoBLL
    {

        private string connStr;
        private string appCode;

        public  AppsInfoBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IAppsInfo infoDAL = AppsInfoFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" AppNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(AppsInfo info)
        {
            try
            {
            if (!ValidateRepeat(info.sAppNo))
            {
                IAppsInfo infoDAL = AppsInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(AppsInfo info)
        {
            try
            {
                     IAppsInfo  infoDAL = AppsInfoFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  AppsInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IAppsInfo infoDAL = AppsInfoFactory.Create(this.connStr, this.appCode);
                AppsInfoCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public AppsInfoCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IAppsInfo infoDAL = AppsInfoFactory.Create(this.connStr, this.appCode);
        ...
        public AppsInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
