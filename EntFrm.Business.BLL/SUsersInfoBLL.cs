...
{
    public class SUsersInfoBLL
    {

        private string connStr;
        private string appCode;

        public  SUsersInfoBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               ISUsersInfo infoDAL = SUsersInfoFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" SuNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(SUsersInfo info)
        {
            try
            {
            if (!ValidateRepeat(info.sSuNo))
            {
                ISUsersInfo infoDAL = SUsersInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(SUsersInfo info)
        {
            try
            {
                     ISUsersInfo  infoDAL = SUsersInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  SUsersInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                ISUsersInfo infoDAL = SUsersInfoFactory.Create(this.connStr, this.appCode);
                SUsersInfoCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public SUsersInfoCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               ISUsersInfo infoDAL = SUsersInfoFactory.Create(this.connStr, this.appCode);
        ...
        public SUsersInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
