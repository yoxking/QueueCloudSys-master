...
{
    public class SysParamsBLL
    {

        private string connStr;
        private string appCode;

        public  SysParamsBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               ISysParams infoDAL = SysParamsFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" ParamNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(SysParams info)
        {
            try
            {
            if (!ValidateRepeat(info.sParamNo))
            {
                ISysParams infoDAL = SysParamsFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(SysParams info)
        {
            try
            {
                     ISysParams  infoDAL = SysParamsFactory.Create(this.connStr, this.appCode);
        ...
        public  SysParams GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                ISysParams infoDAL = SysParamsFactory.Create(this.connStr, this.appCode);
                SysParamsCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public SysParamsCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               ISysParams infoDAL = SysParamsFactory.Create(this.connStr, this.appCode);
        ...
        public SysParamsCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
