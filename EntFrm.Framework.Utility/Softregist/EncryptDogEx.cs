...
namespace EntFrm.Framework.Utility
{
    ...
    public class EncryptDogEx
    {
        ...
        ///</summary>
        ///<returns></returns>
        public string getRegisteNum(string strMNum)
        {
            try
            {
                setIntCode();
        ...
        public string getNewRegNum(string sCode, string sDate,int iSpace=3)
        {
            try
            {
        ...
        public bool doInitEntDog(string encKey, string wHKey, string wLKey, string rHKey, string rLKey)
        {
            string keyPath = "";

            //这个用于判断系统中是否存在着加密锁。不需要是指定的加密锁, 
            if (FindPort(0, ref keyPath) != 0)
            {
            ...
            //先设置写密码
            ret = SetWritePassword("00000000", "00000000", wHKey, wLKey, keyPath);
            if (ret != 0)
            {
                return false;
            }
            ...
            ret = SetReadPassword(wHKey, wLKey, rHKey, rLKey, keyPath);
            if (ret != 0)
            {
                return false;
            }
        ...
        '   提示2: ID唯一是指两个ID转化为16进制字符串后并连接起来后是唯一的*/
            int id_1 = 0, id_2 = 0;
            if (GetID(ref id_1, ref id_2, keyPath) != 0)
            {
                return null;
            }
            return id_1.ToString("X8") + id_2.ToString("X8");
        }

        public short getEntDogVer()
        {
            string keyPath = "";
            ...
            short Version = 0;
            {
                if (NT_GetIDVersion(ref Version, keyPath) != 0)
                {
                    return 0;
                }
            }
