...
namespace EntCodeGener
{
    public partial class CodeGenerator : Form
    {
        ...
        private void btnIndex_Click(object sender, EventArgs e)
        {
            ...
            string sFileDir = Environment.CurrentDirectory.ToString();
            string sFileName = "\\NONCLUSTERED.sql";

            for (int i = 0; i < cbTables.Items.Count; i++)
            {

                sTableName = cbTables.GetItemText(cbTables.Items[i]);

                GetTableInfo(sTableName, ref dt);

                sColName = dt.Rows[1]["ColumnName"].ToString();

                sb.Append("CREATE UNIQUE NONCLUSTERED INDEX Idx_" + sTableName + " ON " + sTableName + "(" + sColName + " ASC ) ");
                sb.Append(" WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, DROP_EXISTING = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON)");
                sb.Append(Environment.NewLine);
