    ...
    public class SUsersController : frmMainController
    {
        ...
        public override ActionResult Add()
        {
            try
            {
                ...
                ViewBag.ItemList = getOrganizList();
            }
            catch (Exception ex)
            {
            }
        ...
        private List<ItemObject> getOrganizList()
        {
            OrganizInfoBLL infoBLL = new OrganizInfoBLL(PublicHelper.Get_ConnStr(), PublicHelper.Get_AppCode());
            OrganizInfoCollections infoColl = infoBLL.GetAllRecords();

            if (infoColl != null && infoColl.Count > 0)
            {
                List<ItemObject> itemList = new List<ItemObject>();
                foreach (OrganizInfo info in infoColl)
                {
                    itemList.Add(new ItemObject(info.sOrganizNo, info.sOrganizName));
                }
