...
namespace EntFrm.Framework.RabbitMq
{
    ...
    public class RabbitMqService : IDisposable
    {
        ...
        /// <param name="isProperties">是否持久化</param>
        /// <param name="handler">消费处理</param>
        /// <param name="isDeadLetter">死信队列</param>
        ...
        /// <param name="handler">消费处理</param>
        /// <param name="isProperties">是否持久化</param>
        public void Pull<T>(string exchange, string queue, string routingKey, Action<T> handler,bool isProperties=false) where T : class
        {
            var channel = GetModel(exchange, queue, routingKey, isProperties);
        ...
        public void RemoveChannel(string queue)
        {
            IModel t = null;
            foreach (var item in ModelDic)
            {
                if (item.Key.Equals(queue))
                {
                    //item.Value.Dispose();
                    ModelDic.TryRemove(queue, out t);
                    t.Dispose();
                }
            }
        }
